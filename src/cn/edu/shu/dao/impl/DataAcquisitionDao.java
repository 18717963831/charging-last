...
public class DataAcquisitionDao<T> implements IDataAcquisitionDao {
	...
	@Override
	public PageBean<Data> getH5yDataPage(PageBean<Data> pb) {
	...
	@Override
	public List<Data> getDataBySdEdDeviceID(String startD, String endD, String deviceid) {
		// TODO Auto-generated method stub
		//找到history表中的addtime在指定范围内并且deviceid指定的数据返回数据集
		
		ArrayList<Data> datas = new ArrayList<Data>();
		String sql="SELECT * FROM historydata WHERE addtime >=? AND addtime <=? AND zl=?";
		try {
			datas = (ArrayList<Data>) qr.query(sql, new BeanListHandler<Data>(
					Data.class), startD, endD,deviceid);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return datas;
